...
=======
		...
		// Item list is not null, so that can add a new item
		assertNotNull("Test if there is valid Cgromebook arraylist to add to", chromebookList);
		...
		ResourceCentre.addChromebook(chromebookList, cb2);
		ResourceCentre.addChromebook(chromebookList, cb3);
		assertEquals("Test that Chrombook arraylist size is 3?", 3, chromebookList.size());
		assertSame("Test that Chromebook is added same as 3rd item of the list?", cb3, chromebookList.get(2));
